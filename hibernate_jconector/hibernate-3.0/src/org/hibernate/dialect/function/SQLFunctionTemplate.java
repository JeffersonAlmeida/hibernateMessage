//$Id: SQLFunctionTemplate.java,v 1.1 2005/02/13 11:49:57 oneovthafew Exp $
 ...
 * E.g. in HQL we can define function <code>concat(?1, ?2)</code> to concatenate two strings p1 and p2.
 * Target SQL function will be dialect-specific, e.g. <code>(?1 || ?2)</code> for Oracle,
 * <code>concat(?1, ?2)</code> for MySql, <code>(?1 + ?2)</code> for MS SQL.
 * Each dialect will define a template as a string (exactly like above) marking function parameters with '?'
 * followed by parameter's index (first index is 1).
 *
