// $Id: Dom4jAccessor.java,v 1.14 2005/02/21 04:29:25 oneovthafew Exp $
...
public class Dom4jAccessor implements PropertyAccessor {
	...
	public static class TextSetter extends Dom4jSetter {
		...
		throws HibernateException {
			...
			if ( !propertyType.isXMLElement() ) { //kinda ugly, but needed for collections with a "." node mapping
				if (value==null) {
					owner.setText(null); //is this ok?
				}
				else {
					propertyType.setToXMLNode(owner, value, factory);
				}
			}
		}
		...
		throws HibernateException {
			if (value!=CollectionType.UNFETCHED_COLLECTION) {
				...
				if (value!=null) {
					...
					propertyType.setToXMLNode(element, value, factory);
				}
			}
		}
