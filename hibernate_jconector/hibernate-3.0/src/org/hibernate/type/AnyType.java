//$Id: AnyType.java,v 1.31 2005/03/16 04:45:24 oneovthafew Exp $
...
import org.hibernate.engine.Cascades.CascadeStyle;
...
public class AnyType extends AbstractType implements AbstractComponentType, AssociationType {
	...
	throws HibernateException {
		return (entityName==null || id==null) ?
			null :
			session.internalLoad(entityName, id);
	}
	...
	throws HibernateException {
		...
		return e==null ? null : session.load(e.entityName, e.id);
	}
	...
	throws HibernateException {
		if (original==null) {
			...
			return session.load( 
					session.bestGuessEntityName(original), 
					ForeignKeys.getEntityIdentifierIfNotUnsaved( 
							session.bestGuessEntityName(original), original, session 
					)
			);
		}
	}
	...
	throws HibernateException {
		...
		return !holder.entityName.equals( session.bestGuessEntityName(current) ) ||
			identifierType.isModified(holder.id, getIdentifier(current, session), session);
	}
