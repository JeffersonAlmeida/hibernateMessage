//$Id: PathExpressionParser.java,v 1.10 2005/02/23 02:31:41 oneovthafew Exp $
...
public class PathExpressionParser implements Parser {
	...
	String continueFromManyToMany(String clazz, String[] joinColumns, QueryTranslatorImpl q) throws QueryException {
		start( q );
		continuation = true;
		currentName = q.createNameFor( clazz );
		q.addType( currentName, clazz );
		Queryable classPersister = q.getEntityPersister( clazz );
		//QueryJoinFragment join = q.createJoinFragment(useThetaStyleJoin);
		addJoin( currentName, ( AssociationType ) Hibernate.entity( clazz ), joinColumns );
