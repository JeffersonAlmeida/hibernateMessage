//$Id: SQLLoaderTest.java,v 1.13 2005/03/19 00:29:14 maxcsaucdk Exp $
...
public class SQLLoaderTest extends TestCase {
	...
	public void testEscapedJDBC() throws HibernateException, SQLException {
		...
		if ( getDialect() instanceof OracleDialect ) {
            ...
            query = session.createSQLQuery("select identifier_column as {a.id}, clazz_discriminata as {a.class}, count_ as {a.count}, name as {a.name} from A where {fn ucase(name)} like {fn ucase('max')}", "a", A.class);
        }
		List list = query.list();
	...
	public void testDoubleAliasing() throws HibernateException, SQLException {
		if ( getDialect() instanceof HSQLDialect || getDialect() instanceof SQLServerDialect ) return;
